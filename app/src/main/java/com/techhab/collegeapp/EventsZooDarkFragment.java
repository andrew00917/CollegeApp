 ...
 */
public class EventsZooDarkFragment extends Fragment {
    ...
    public EventsZooDarkFragment() {
        // Required Empty Constructor
    }

    public static Fragment createNewInstance() {
        EventsZooDarkFragment fragment = new EventsZooDarkFragment();
    ...
    public List<EventsRssItem> getZooDarkEvent(List<EventsRssItem> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            item = list.get(i);
            if (item.getEvent().contains("After Dark")) {
                ret.add(item);
            }
        }
        return ret;
